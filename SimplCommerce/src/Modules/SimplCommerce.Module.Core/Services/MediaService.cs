...
namespace SimplCommerce.Module.Core.Services
{
    public class MediaService : IMediaService
    {
        ...
        public string GetThumbnailUrl(Media media, string size)
    {
        if (media == null)
        {
            return _storageService.GetMediaUrl("no-image.png");
        }

        // SimplCommerce StorageService usually returns /user-content/filename
        // We want to return /images/{size}/filename
        return $"/images/{size}/{media.FileName}";
    }

    // Update the original interface method to use the default size
    public string GetThumbnailUrl(Media media)
    {
        return GetThumbnailUrl(media, "320x320xi");
    }
